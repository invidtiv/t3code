...
const DEFAULT_DEEPSEEK_TURN_INACTIVITY_TIMEOUT_MS = 10 * 60 * 1_000;
const DEFAULT_DEEPSEEK_ACTIVE_TOOL_INACTIVITY_TIMEOUT_MS = 30 * 60 * 1_000;
...
interface DeepSeekTurnLivenessSignal {
...
interface DeepSeekSessionContext {
  ...
  readonly livenessSignals: Queue.Queue<DeepSeekTurnLivenessSignal>;
...
const resolveNotificationTurnId = (ctx: DeepSeekSessionContext): TurnId | undefined =>
  ctx.activeTurnId;

function parseDeepSeekResume(raw: unknown): { sessionId: string } | undefined {
  ...
  return Effect.gen(function* () {
    ...
    const turnInactivityTimeoutMs =
      ...
      Number.isFinite(requestedTurnInactivityTimeoutMs)
        ...
        : DEFAULT_DEEPSEEK_TURN_INACTIVITY_TIMEOUT_MS;
    ...
    const activeToolInactivityTimeoutMs =
      ...
      Number.isFinite(requestedActiveToolInactivityTimeoutMs)
        ...
        : DEFAULT_DEEPSEEK_ACTIVE_TOOL_INACTIVITY_TIMEOUT_MS;
    ...
    const beginTurnLiveness = (ctx: DeepSeekSessionContext, turnId: TurnId) =>
      Effect.sync(() => {
    ...
    const clearTurnLiveness = (ctx: DeepSeekSessionContext) => {
    ...
    const recordTurnActivity = Effect.fn("DeepSeekAdapter.recordTurnActivity")(function* (
      ctx: DeepSeekSessionContext,
      turnId: TurnId,
      event: Extract<
    ...
    const hasLivenessPause = (ctx: DeepSeekSessionContext) =>
      ctx.pendingApprovals.size > 0 ||
      ctx.pendingUserInputs.size > 0 ||
      ctx.livenessUpdatesInFlight > 0;

    const livenessTimeoutFor = (ctx: DeepSeekSessionContext) =>
    ...
    const resumeSessionTurnLiveness = Effect.fn("DeepSeekAdapter.resumeSessionTurnLiveness")(
    ...
    const refreshSessionTurnLiveness = Effect.fn("DeepSeekAdapter.refreshSessionTurnLiveness")(
      function* (threadId: ThreadId, turnId: TurnId | undefined) {
        const ctx = sessions.get(threadId);
        if (
          !ctx ||
    ...
    const markPromptResponseReady = Effect.fn("DeepSeekAdapter.markPromptResponseReady")(function* (
      threadId: ThreadId,
      acpSessionId: string,
      turnId: TurnId,
    ) {
    ...
    const consumePromptResponseReady = (ctx: DeepSeekSessionContext) => {
      ctx.promptResponsesReady = Math.max(0, ctx.promptResponsesReady - 1);
    };

    const isLiveTurn = (ctx: DeepSeekSessionContext, turnId: TurnId) =>
    ...
    const settleStalledTurn = Effect.fn("DeepSeekAdapter.settleStalledTurn")(function* (
      ctx: DeepSeekSessionContext,
      turnId: TurnId,
    ) {
      return yield* withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          yield* settlePromptInFlight(ctx.threadId, turnId, ctx.acpSessionId, {
            errorMessage: `DeepSeek ACP turn stalled without content or tool progress for ${livenessTimeoutFor(ctx).milliseconds}ms.`,
    ...
    const runTurnLivenessWatchdog = Effect.fn("DeepSeekAdapter.runTurnLivenessWatchdog")(
      function* (ctx: DeepSeekSessionContext) {
        while (true) {
    ...
    const logNative = (threadId: ThreadId, method: string, payload: unknown) =>
      Effect.gen(function* () {
        ...
        Effect.catchCause((cause) =>
          Effect.logWarning("Failed to write native DeepSeek notification log.", {
            cause,
            threadId,
            method,
          }),
    ...
    const startSession: DeepSeekAdapterShape["startSession"] = (input) =>
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const resumeSessionId = parseDeepSeekResume(input.resumeCursor)?.sessionId;
          ...
          const ctx: DeepSeekSessionContext = {
            ...
            livenessSignals: yield* Queue.sliding<DeepSeekTurnLivenessSignal>(1),
            ...
            return yield* Effect.gen(function* () {
              ...
              if (ctx.interruptedTurnIds.has(turnId)) {
                ...
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "DeepSeek prompt was interrupted during preparation.",
                });
              }
              ...
              Effect.tapCause(() =>
                Effect.gen(function* () {
                  ...
                  yield* settlePromptInFlight(input.threadId, turnId, liveCtx.acpSessionId, {
                    errorMessage: "DeepSeek prompt preparation failed.",
                    emitTurnCompletion: false,
                  });
                }),
              ),
            ...
            Effect.gen(function* () {
              ...
              if (ctx.acpSessionId !== prepared.acpSessionId) {
                yield* settlePromptInFlight(
                  ...
                  prepared.acpSessionId,
                  {
                    errorMessage: "DeepSeek session changed before the turn completed.",
                    settleAllPrompts: true,
                  },
                );
                yield* Ref.set(promptSettled, true);
                return yield* new ProviderAdapterRequestError({
                  provider: PROVIDER,
                  method: "session/prompt",
                  detail: "DeepSeek session changed before the turn completed.",
                });
              }
              for (let yieldAttempt = 0; yieldAttempt < 8; yieldAttempt += 1) {
                yield* Effect.yieldNow;
            ...
            Effect.gen(function* () {
              ...
              yield* withThreadLock(
                ...
                settlePromptInFlight(input.threadId, prepared.turnId, prepared.acpSessionId, {
                  errorMessage: errorMessage ?? "DeepSeek prompt request failed.",
                }),
              );
    ...
    const rollbackThread: DeepSeekAdapterShape["rollbackThread"] = (threadId, numTurns) =>
      Effect.gen(function* () {
        ...
        return yield* new ProviderAdapterRequestError({
          ...
          detail: "DeepSeek ACP sessions do not support provider-side rollback yet.",
        });
      });
